...
| 维度 | Cyera | DataDNA | 取舍 |
|------|------|------|------|
| 架构 | 多引擎并行融合 (4引擎) | 多引擎并行融合 (6引擎) | 继承融合思想，引擎更多样 |
| 元数据替换 | 有 (PII替换为类型占位符) | E2模板引擎沿用此方案 | 借鉴，实现方式独立 |
| LLM | 无 (专利不含LLM) | E6引擎，权重最高 | DataDNA独有，零样本能力 |
| 数据发现 | 有 (快照扫描、惰性挂载) | 无 | 分属不同问题域 |
| 新类型处理 | 人工扩展固定分类体系 | LLM自动+人工确认，动态类型库 | DataDNA更灵活 |
| 融合方式 | 真值表融合 | 加权投票，LLM置信度加权² | 本质不同算法 |
| 效率 | ~8ms/文档 (所有引擎并行) | 2ms(共识) 或 ~1.5s(含LLM) | 各有优势 |
| LLM不可用时 | N/A (无LLM) | E1-E5融合，系统仍可用 | DataDNA的P5原则 |
| 可解释性 | 中 (融合结果) | 高 (每引擎输出独立可审计) | DataDNA更透明 |
| 专利风险 | — | 经要素对比分析，不侵权 | 见专利分析文档 |

**专利分析结论** (8项专利逐一要素对比): DataDNA是分类引擎，不涉及Cyera数据发现专利(US12026123等)中的快照扫描、惰性挂载、条件克隆等要素。E2模板引擎借鉴了元数据替换的思想，但实现路径不同(PII正则+SHA256 vs 元数据快照)。真值表融合与加权投票在数学上不同。整体不构成字面侵权。
