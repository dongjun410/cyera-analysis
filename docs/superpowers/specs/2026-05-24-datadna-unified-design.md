...
```
                          ┌──────────────────────────────────────┐
                          │           DataDNA 分类引擎             │
                          │      6-Engine Parallel Fusion         │
                          └────────────────┬─────────────────────┘
                                           │
             ┌─────────────────────────────┼─────────────────────────────┐
             │                             │                             │
    ┌────────▼────────┐           ┌────────▼────────┐           ┌────────▼────────┐
    │  E1 正则规则     │           │  E2 模板Hash     │           │  E3 ML (SetFit)  │
    │  55条类型规则    │           │  PII替换→SHA256  │           │  教师: LLM       │
    │  延迟 <1ms       │           │  延迟 <1ms       │           │  延迟 ~2ms       │
    │  权重 1.0        │           │  权重 1.0        │           │  权重 1.5        │
    └────────┬────────┘           └────────┬────────┘           └────────┬────────┘
             │                             │                             │
    ┌────────▼────────┐           ┌────────▼────────┐           ┌────────▼────────┐
    │  E4 语义kNN      │           │  E5 结构签名     │           │  E6 LLM          │
    │  BGE-M3→质心匹配 │           │  元数据→SHA256   │           │  qwen/mistral    │
    │  延迟 ~2ms       │           │  延迟 <1ms       │           │  延迟 ~1.5s      │
    │  权重 1.0        │           │  权重 0.8        │           │  权重 2.0        │
    └────────┬────────┘           └────────┬────────┘           └────────┬────────┘
             │                             │                             │
             └─────────────────────────────┼─────────────────────────────┘
                                           │
                          ...
                          │                │                │
                   ┌──────▼──────┐  ┌──────▼──────┐  ┌──────▼──────┐
                   │ conf≥0.85   │  │ conf<0.85   │  │ conf<0.4    │
                   │ fusion_fast │  │ fusion_full │  │ manual_     │
                   │ 跳过LLM     │  │ 调用E6      │  │ review      │
                   │ ~5ms        │  │ ~1.5s       │  │ 人工审核     │
                   └─────────────┘  └─────────────┘  └─────────────┘
                              │                │
...
**核心流程**: 6引擎并行运行 → 加权投票融合 → 共识阈值判断 → LLM调用优化 → 审计记录。每个引擎独立输入、独立输出、独立故障。
